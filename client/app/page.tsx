...
export default function Home() {
  ...
  const { state, sendMessage, getThinkingTrace } = useAgentStream();
          ...
          {state.messages.length === 0 ? (
            <div className="mt-24 text-center text-gray-400">
              ...
              <p className="mt-2 text-sm">
                Try: “Weather in Tokyo”, “345 * 99 + 120 / 6”, or “latest AI news”
              </p>
            </div>
          ) : (
              ...
              {state.messages.map((message, index) => {
                ...
                return (
                  <ChatMessage
                    key={message.id}
                    message={message}
                    graphNodes={isFinalAssistantText ? state.graphNodes : undefined}
                    thinkingTrace={isFinalAssistantText ? getThinkingTrace() : undefined}
                  />
                );
              })}
              <div ref={endRef} />
